...
def val(net, val_ds, muti_bce_loss_fusion):
    ...
    for i, data in enumerate(val_ds):        
        ...
        print("[sub ite: %d, loss: %4f, tar: %4f, iou: %4f]" %(i, loss.data.item(), _, iou.mean()))
        # del temporary outputs and loss
        del d0, d1, d2, d3, d4, d5, d6, loss, _
 
        
    return total_loss/(i+1), iou.mean()
    ...
    for epoch in range(0, epoch_num):
        ...
        for i, data in enumerate(train_ds):
            ...
            if ite_num % save_frq == 0:
                ...
                val_loss, val_iou = val(net, val_ds, muti_bce_loss_fusion)
                wandb.log({
                    "val_loss":val_loss,
                    "val_iou":val_iou
                })
                torch.save(net.state_dict(), "./saved_models/" + trail_name +"/_bce_itr_%d_train_%3f_tar_%3f_val_loss_%4f_val_iou_%4f.pth" % (ite_num, running_loss / ite_num4val, running_tar_loss / ite_num4val, val_loss, val_iou))
                running_loss = 0.0
                running_tar_loss = 0.0
